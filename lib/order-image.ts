...
export function orderImageUrl(order: OrderImageSource | null | undefined): string {
  return (
    usableImageSrc(order?.shop_image_url) ??
    usableImageSrc(order?.first_item_image_url) ??
 ...
 * a server-side bug, but a page must not blank out because of one.
 */
export function usableImageSrc(value: string | null | undefined): string | null {
